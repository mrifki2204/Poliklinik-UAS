                    ...
                    <header class="flex items-center justify-between">
                        ...
                        <div class="flex-col items-center justify-center text-center">
                            ...
                            @if (session('status') === 'jadwal-periksa-exists')
                                <p class="text-sm text-gray-600">{{ __('Exists.') }}</p>
                            @endif
                        </div>
                    </header>
                        ...
                        <tbody>
                            @foreach ($jadwalPeriksas as $jadwalPeriksa)
                                <tr>
                                    ...
                                    <td class="align-middle text-start">
                                        <form action="{{ route('dokter.jadwal-periksa.update', $jadwalPeriksa->id) }}" method="POST" style="display: inline-block; margin-right: 5px;">
                                            @csrf
                                            @method('PATCH')
                                            @if (!$jadwalPeriksa->status)
                                        ...
                                        <form action="{{ route('dokter.jadwal-periksa.destroy', $jadwalPeriksa->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jadwal ini?');">
                                            @csrf
                                            @method('DELETE')
